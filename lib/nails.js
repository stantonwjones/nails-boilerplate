...
var Controller = require('./controller_es5.js');
...
function configure( app_config ) {
    ...
    // Init view engine
    cons.requires[app_config.config.VIEW_ENGINE] = require(app_config.config.VIEW_ENGINE_PATH);
    // TODO: make react the default and allow override from others
    express_app.engine('tmpl', cons[app_config.config.VIEW_ENGINE]);
    express_app.engine('jsx', require('express-react-views').createEngine());
    express_app.set('views', app_config.config.VIEWS_ROOT);
    express_app.set('view engine', 'tmpl');
...
function startServer(config) {
    ...
    if (application.config.PORT) {
      express_app.listen(application.config.PORT);
    } else {
...
function init_app_lib(base_constructor, abs_path) {
    ...
    if (fs.statSync(abs_path).isFile())
      return base_constructor.extend( require(abs_path) );
