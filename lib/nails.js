...
function configure( app_config ) {
    ...
    // TODO: init internal views and controllers by importing them from application/views
    ...
    var tmpdir = app_config.config.UPLOAD_DIR || path.join(app_config.config.SERVER_ROOT, 'tmp');
    application.router.setUploadDir( tmpdir );
    ...
    // TODO: init_models
    //  should cycle through models in models folder and extend them
    //  (?) how are models made available to controller?
    //  through an object on the request/response...
    //  (?) should they also be made available to other objects?
};
...
function startServer(config) {
    ...
    var server = http.createServer( function( req, res ) {
        ...
        req_domain.add(req);
        req_domain.add(res);
        req_domain.on('error', function(exception) {
            console.log('SERVER ERROR: ', exception.message + '\n\n' + exception.stack);
            if (res.headersSent) return;
            application.controller._do( '500', {error: exception}, req, res );
        });
        Cookies.wrap(req, res);
        // TODO: optional session would go here
        // TODO: wrap request with a respondWith function which calls a view
    ...
    var server_domain = domain.create();
    server_domain.on('error', function(er) {
        console.error('\nCaught toplevel exception', exception.message + '\n\n' + exception.stack);
    });
    server_domain.run(function() {
