...
function JSONView(){};
function PublicView(){
    this.public_root = path.resolve(application.config.SERVER_ROOT, application.config.PUBLIC_ROOT);
};
PublicView.prototype.render = function(params, response) {
    var file_path = params.path;
    if (file_path.substr(0, this.public_root.length) != this.public_root)
        file_path = path.join(this.public_root, file_path);
