...
function MongoDBConnector(config) {
    ...
    var url = config.url || 'localhost';
    var port = config.port || '27017';
    var database = config.database || 'nails';
    this.exec_once_connected = [];
    this.server_connection = mongodb.Server(url, port);
    this._db = new mongodb.Db(database,
...
MongoDBConnector.prototype.post = function(model_or_collection) {
    if (model_or_collection.is_model) 
        this._post_one(model_or_collection._collection_name(), model_or_collection.attributes);
}
MongoDBConnector.prototype._post_one = function(collection_name, doc_attributes) {
...
MongoDBConnector.prototype.get = function(model_or_collection) {
    if (model_or_collection.is_model)
        this._get_one(model_or_collection._collection_name(), model_or_collection.id, null,
                this._on_get.bind(model_or_collection));
}
...
MongoDBConnector.prototype._on_get = function(err, doc) {
