...
Router.prototype.route = function( request, response ) {
    ...
    if ( !request.parsed_form && request.method.match(/^(post|put)$/i) ) this.handle_incoming_form(request, response);
    /*
    if ( !request.parsed_form && request.method.match(/^(post|put)$/i) ) {
        ...
        form.on('error', console.log.bind(console));
    ...
    */
    ...
    var routeParams = this.get_dispatch_params( method, uri.pathname, querystring.parse(uri.query), request.form, request.files );
...
Router.prototype.handle_incoming_form = function(request, response) {
    ...
    form.uploadDir = this.upload_dir;
    form.keepExtensions = true;
    form.on('\nFORM ERROR:', console.log.bind(console));
    return form.parse(request, this.handle_parsed_form.bind(this, request, response) );
}
...
Router.prototype.get_dispatch_params = function( method, path, query, form, files) {
    ...
    _.extend(params.params, query, form, files);
